        ...
        <form id="searchOrder" action="{{ url()->current() }}" method="GET">
            <div class="flex card-header justify-content-betweens">
                <div class="col-9">
                    ...
                    <div
                        ...
                        >
                            ...
                            <select name="role" class="form-control aiz-selectpicker" data-live-search="true" id="">
                                <option value="">Select Role</option>
                                <option value="euthor">Author</option>
                ...
                <div class="">
                    ...
                    <a href="{{ route('admin.report.purchaseOrder.index') }}" id="reset" class="btn btn-warning">Reset</a>
                    @if(request('from_date') && request('to_date'))
                        <a href="{{url('/admin/bb-b-05b-pdf')}}?from_date={{request('from_date')}}&to_date={{request('to_date')}}" target="_blank" class="btn btn-success">
                           PDF
                        </a>
                    @endif
                </div>
         ...
         @foreach ($data as $item)
            <div class="row justify-content-start">
                <div class="col-md-3">
                    <div class="text-black bg-info" style="border-radius: 2px; margin:10px;">
                        <div class="text-left" style="font-size: 17px; margin:5px;">Publisher :  <b>{{ @request('role') }}</b></div>
                    </div>
                </div>
            </div>
        <table class="table aiz-table table-sm">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($item['data'] as $key => $value)
                    <tr>
                        <td style="text-align: center" width="10%">{{ ++ $key}}</td>
                        <td style="text-align: center" width="10%">{{$value->orderNo }}</td>
                        <td style="text-align: left" width="20%">{{ $value->bookNameBn }}</td>
                        <td style="text-align: center" width="10%">{{ $value->qty }}</td>
                        <td style="text-align: center" width="10%">Pcs</td>
                        <td style="text-align: center" width="10%">{{ $value->rate }}</td>
                        <td style="text-align: center" width="10%">{{ $value->amount }}</td>
                        <td style="text-align: left" width="20%">{{ $value->remarks }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
