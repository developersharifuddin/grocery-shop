        ...
        <div class="col-12 p-0">
            <div class="card">
                <div class="card-header justify-content-between py-3">
                    <h4 class="card-title float-left pt-2">All Sales</h4>
                    <div class="float-right d-flex">
                        <form action="{{ route('admin.sales.index') }}" method="GET"
                            ...
                            <div class="form-group mb-0 d-flex w-auto">
                                <input type="date" name="from_date" class="form-control mr-2" id="inputGroupFile02"
                                    placeholder="from_date" value="{{ request('from_date') }}">
                                <input type="date" name="to_date" class="form-control" id="inputGroupFile02"
                                    placeholder="to_date" value="{{ request('to_date') }}">
                            </div>
                        ...
                        <tbody id="datatable">
                            @if (count($sells) > 0)
                                @foreach ($sells as $key => $sale)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        {{-- <td>{{ $sale->firstItem() + $loop->index }}</td> --}}
                                        {{-- <td>{{ $sale->total() - $sale->perPage() * ($sale->currentPage() - 1) - $loop->index }}</td> --}}
                                        ...
                                        {{-- <td>{{ number_format($sale->payments->total_paid_amount, 2) }}</td> --}}
                                        ...
                                        @foreach ($sale->payments as $payment)
                                            <td>
                                                @php
                                                    $badgeClass = $payment->total_due_amount === 0.0 ? 'badge badge-success' : 'badge badge-danger';
                                        ...
                                        <td>
                                            ...
                                            @endphp
                                            <span
                                                class="{{ $badgeClass }}">{{ $sale->payment_status == '1' ? 'Paid' : 'Unpaid' }}</span>
                                        </td>


                                        <td>
                                            {{-- <a href="{{ route('admin.sales.edit', $sale->id) }}"
                                                class="btn btn-primary btn-sm edit text-light border-0" title="Edit">
                                ...
                                <button type="submit" class="btn btn-sm btn-danger delete" onclick="return confirm('Are you sure?')"> <i class="fas fa-trash-can"></i></button>
                                    ...
                                    <i class="fa-regular fa-pen-to-square"></i>
                                            ...
                                            <div class="btn-group bg-transparent dropleft" title="View Account">
                                                <a class="btn btn-o dropdown-toggle btn btn-info btn-sm text-light view border-0"
                                                    data-toggle="dropdown" href="#"> Options <span
                                                        class="caret"></span>
                                                </a>
                                                <ul role="menu" class="dropdown-menu pull-left px-3 py-2"
                                                    style="min-width:200px">
                                                    <li class="py-1">
                                                        <a title="View Invoice" class="dropdown-item"
                                                            href="{{ route('admin.sales.show', $sale->id) }}">
                                                            <i class="fa fa-fw fa-eye text-blue"></i> View Sales
                                                        </a>
                                                    </li>
                                                    {{-- <li class="py-1">
                                                        <a title="Update Record ?" class="dropdown-item" href="pos/edit/41">
                                                            <i class="fa fa-fw fa-edit text-blue"></i>Edit
                                                        </a>
                                                    </li> --}}
                                                    <li class="py-1">
                                                        <a title="Pay" class="dropdown-item" onclick="view_payments(41)">
                                                            <i class="fa-solid fa-money-bill text-blue"></i> View Payments
                                                        </a>
                                                    </li>
                                                    <li class="py-1">
                                                        <a title="Pay" class="dropdown-item" onclick="pay_now(41)">
                                                            <i class="fa fa-fw fa-hourglass-half text-blue"></i> Payment
                                                            Receive
                                                        </a>
                                                    </li>
                                                    <li class="py-1">
                                                        <a title="Update Record ?" class="dropdown-item" target="_blank"
                                                            href="sales/print_invoice/41">
                                ...
                                <form method="POST" action="{{ route('admin.sales.store') }}"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name_english"
                                            ...
                                            id="name_english" value="{{ old('name_english') }}" required>
                                        @error('name_english')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name_bangla"
                                            ...
                                            id="name_bangla" value="{{ old('name_bangla') }}" required>
                                        @error('name_bangla')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="parent_id"
                                            class="form-control @error('parent_id') is-invalid @enderror" id="parent_id">
                                            <option value="">Select Parent sales</option>
                                            @foreach ($sells as $sell)
                                                <option value="{{ $sell->id }}"
                                                    {{ old('parent_id') == $sell->id ? 'selected' : '' }}>
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-4">
                                    ...
                                    <input class="form-control" type="text" name="HID-date"
                                        placeholder="sales Name English..." value="Fresh Water">
                                </div>
                                <div class="mb-3 col-md-4">
                                    <label>sales Name Bangla</label>
                                    <input class="form-control" type="text" name="HID-date"
                                        placeholder="sales Name bangla..." value="Fresh Water">
                                </div>
                        ...
                        <div class="modal-body px-md-4">
                            <div class="row">
                                <div class="mb-3 col-md-4">
                                    <label>sales Name</label>
                                    <input class="form-control" type="text" name="HID-date"
                                        placeholder="sales Name English..." value="Fresh Water" disabled>
                                </div>
                                <div class="mb-3 col-md-4">
                                    <label>sales Name Bangla</label>
                                    <input class="form-control" type="text" name="HID-date"
                                        placeholder="sales Name bangla..." value="Fresh Water" disabled>
                                </div>
